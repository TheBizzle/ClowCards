...
define(['r/adt/iterator', 'r/adt/option', 'r/api/underscore'], (Iterator, Opt, _) ->
  ...
  test("maxBy", ->
    ...
    deepEqual(iterator.clone().maxBy((x) -> if x is 21 then 9001 else if x is 89 then -9001 else 0), 21)
  )
  ...
  test("minBy", ->
    ...
    deepEqual(iterator.clone().minBy((x) -> if x is 21 then 9001 else if x is 89 then -9001 else 0), 89)
  )
  ...
  test("sortBy", ->
    ...
    deepEqual(iterator.clone().sortBy((x) -> -x),                x for x in [100..1])
    deepEqual(iterator.clone().sortBy((x) -> (x + "").length),   [1..100])
    deepEqual(iterator.clone().sortBy((x) -> -Math.abs(x - 50)), zipped)
    deepEqual(iterator.clone().sortBy((x) -> if x is 21 then 9001 else if x is 89 then -9001 else 0), [89].concat(_([1..100]).filter((x) -> not (x is 21 or x is 89)).concat([21])))

  )

  test("groupBy", ->
    deepEqual(iterator.clone().groupBy((x) -> x),               _([1..100]).foldl(((acc, x) -> acc[x] = [x]; acc), {}))
    deepEqual(iterator.clone().groupBy((x) -> (x + "").length), { 1: [1..9], 2: [10..99], 3: [100] })
    deepEqual(iterator.clone().groupBy((x) -> if x is 21 then 9001 else if x is 89 then -9001 else 0), { '-9001': [89], 0: _([1..100]).filter((x) -> not (x is 21 or x is 89)), 9001: [21] })
  )
  ...
  test("Mixed Ops", ->
    deepEqual(iterator.clone().map((x) -> x * 3).filter((x) -> x % 2 is 1).filterNot((x) -> x < 10).map((x) -> x - 2).next(), 13)
    deepEqual(iterator.clone().map((x) -> x + 1).drop(10).filter((x) -> (x + "").length is 3).toArray(),                      [100..101])
    deepEqual(iterator.clone().filter((x) -> x % 2 is 0).drop(20).map((x) -> x * 10).take(10),                                _(x for x in [420..1000] by 20).take(10))
  )
